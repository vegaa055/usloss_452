...
int XXp1(char *arg)
{
  ...
  for (i = 0; i < 3; i++)
    pid[i] = fork1("XXp2", XXp2, "XXp2", USLOSS_MIN_STACK, 2);
...
int XXp2(char *arg)
{
  ...
  printf("XXp2(): started, pid = %d, calling block_me\n", getpid());
  result = block_me(19);
  printf("XXp2(): pid = %d, after block_me, result = %d\n", getpid(), result);
  printf("XXp2(): pid = %d, is_zapped() = %d\n", getpid(), is_zapped());
  quit(0);
  return 0;
} /* XXp2 */
